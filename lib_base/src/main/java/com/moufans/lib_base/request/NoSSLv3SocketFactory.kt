package com.moufans.takephotos.request
...
class NoSSLv3SocketFactory : SSLSocketFactory {
    ...
    private val myProtocols = arrayOf("TLSv1", "TLSv1.1", "TLSv1.2")
    ...
    fun makeSocketSafe(socket: Socket): Socket {
        var socket = socket
    ...
    inner class NoSSLv3SSLSocket(delegate: SSLSocket) : DelegateSSLSocket(delegate) {

        override fun setEnabledProtocols(protocols: Array<String>) {
            if ( protocols.size == 1 && "SSLv3" == protocols[0]) {
            ...
            if ( protocols.size == 1 && "TLSv1" == protocols[0]) {
